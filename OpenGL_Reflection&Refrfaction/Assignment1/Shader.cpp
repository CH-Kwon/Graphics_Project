	...
	m_infoLogLength(0), m_result(GL_FALSE), isGeoUse(GeoUse), m_norm(type)
{
	...
	else if(type == PHONG_SHADING)
	{
		CreateShader("shaders\\phong_shading_vert.vert", "shaders\\phong_shading_frag.frag", Unreferenced);
	}

	else if (type == CUBEMAP)
	{
		CreateShader("shaders\\cubemapping.vert", "shaders\\cubemapping.frag", Unreferenced);
	}
	else if (type == SKYBOX)
	{
		CreateShader("shaders\\skybox.vert", "shaders\\skybox.frag", Unreferenced);
	}
}
...
void Shader::CreateShader(const char* vert_path, const char* frag_path, const char* geo_path)
{
	...
	if (isGeoUse == true)
	{
		...
		if (m_result == GL_FALSE) 
		{
			glGetShaderiv(m_geometryId, GL_INFO_LOG_LENGTH, &m_infoLogLength);
			std::vector<char> ShaderErrorMessage(m_infoLogLength + 1);
			glGetShaderInfoLog(m_geometryId, 1024, &m_infoLogLength, &ShaderErrorMessage[0]);
			printf("Geo Shader - %4s\n", &ShaderErrorMessage[0]);
		}
	}
